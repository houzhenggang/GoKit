...
- (void)readPCMAndPlay:(AudioQueueRef)outQ buffer:(AudioQueueBufferRef)outQB {
    NSUInteger rlen;
    NSUInteger num = (_stream.length-readLength);
    ...
    NSLog(@"total:%lu read:%lu cur:%lu", (unsigned long)_stream.length, (unsigned long)readLength, (unsigned long)rlen);
    
    outQB->mAudioDataByteSize = (UInt32)rlen;
    Byte *audiodata = (Byte *)outQB->mAudioData;
    for (int i=0; i<rlen; i++) {
        audiodata[i] = ((unsigned char *)_stream.bytes)[readLength+i];
    }
    readLength += rlen;
    /*
