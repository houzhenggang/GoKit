...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *nextStep = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Resend", @"LocalizedSimpleChinese", nil) style:UIBarButtonItemStyleDone target:self action:@selector(resend)];
    UIBarButtonItem *space = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    [self setToolbarItems:@[space, nextStep, space] animated:YES];
    [self.navigationController setToolbarHidden:NO animated:YES];
    
    [self.navigationItem setTitle:NSLocalizedStringFromTable(@"AdjustVolume", @"LocalizedSimpleChinese", nil)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (0 == section) {
        ...
        [label setText:NSLocalizedStringFromTable(@"CurrentVolume", @"LocalizedSimpleChinese", nil)];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (0 == section) {
        ...
        [label setTextColor:[UIColor grayColor]];
        [label setFont:[UIFont systemFontOfSize:15]];
        [label setText:NSLocalizedStringFromTable(@"AdjustVolumeHint", @"LocalizedSimpleChinese", nil)];
